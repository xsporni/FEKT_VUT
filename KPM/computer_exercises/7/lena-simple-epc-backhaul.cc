...
// definition of logging component with the name LenaSimpleEpcBackhaul
...
int main(int argc, char *argv[]) 
{
	...
	CommandLine cmd;//premenna objektu CommandLine
    ...
    // creating a packet gateway through the helper obejct
    ...
    // Assigning the connection between two nodes and setting up the IP address
    ...
    // Installing Mobility Model for SGW
	...
	if (!useHelper) 
    {
		...
		for (uint16_t i = 0; i < numNodePairs; ++i) {
            ...
            // Creae a point to point link between the eNB and the SGW with the corresponding new NEtDevices
            // on each side
			PointToPointHelper p2ph;
            ...
            // Creating S1 interface between the SGW and the eNB
			epcHelper->AddS1Interface(enb, enbS1uAddress, sgwS1uAddress);
		}
	}
    // Installing the IP stack on th UEs
    ...
    // Install and start applications on the UEs and remote host
	...
	// application start and logging turned on
